...
public class AntennaArray {
   ...
   */
    ...
    public int getNumberOfAntenna()
    {
        return n_antennae;
    }

    public boolean inBounds(double[] vector)
    {
        double[] vectorToTest = Arrays.copyOf(vector, vector.length);
        Arrays.sort(vectorToTest);
        for(int i = 0; i < vectorToTest.length - 1; ++i)
            if(vectorToTest[i] < bounds()[i][0] || vectorToTest[i] > bounds()[i][1] )
                return false;
        return true;
    }
}
